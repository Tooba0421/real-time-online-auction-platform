...
const BidderManagement = () => {
  ...
  const handleApprove = async (submission) => {
    try {
      ...
      if (buyerErr) {
        ...
        console.error("Buyer insert error:", buyerErr);
        return;
      }
  ...
  const handleConfirmReject = async () => {
    ...
    try {
      ...
      // Step 1: Mark submission rejected
      await supabase
        .from("pending_cnic_submissions")
        .update({ status: "rejected" })
        .eq("id", selectedBidder.submissionId);

      // Step 2: FIX — also update profiles.id_verified so user knows they were rejected
      await supabase
        .from("profiles")
        .update({ id_verified: "rejected" })
        .eq("id", selectedBidder.user_id);

      // Step 3: Store reason in admin_actions (no rejection_reason column in table)
    ...
    try {
      ...
      await Promise.all([
        ...
        supabase.from("notifications").insert({
          ...
          message:          "Your CNIC update request has been approved. Your information has been updated.",
          type:             "approval",
          notification_for: "buyer",
          is_read:          false,
        }),
  ...
  return (
    <div className="admin-page">
                  ...
                  : pendingBuyerEdits.map((edit) => (
                    <tr key={edit.id}>
                      ...
                      <td className="actions">
                        ...
                        <ActionButton label="Reject" variant="danger"   onClick={() => { setEditRejectModal(edit); setEditRejectReason(""); }} disabled={processing} />
                      </td>
                    </tr>
                  ))}
              </tbody>
                ...
                {approvedBuyers.length === 0
                  ...
                  : approvedBuyers.map((buyer) => (
                    <tr key={buyer.id}>
                      ...
                      <td>
                        {/* Show account status — ban/suspend is managed from User Management */}
        ...
        {biddersLoading ? (
          ...
          <div className="table-wrapper">
            <table className="admin-table">
              <thead>
                <tr>
                  <th>Name</th><th>Email</th><th>CNIC No</th><th>Status</th><th>Submitted</th>
                </tr>
              </thead>
              <tbody>
      ...
      {/* ── CNIC VIEW MODAL ──────────────────────────────────────── */}
      {selectedCnic && (
        <div className="cnic-modal-overlay" onClick={() => { setSelectedCnic(null); setCnicUrls({ front: null, back: null }); }}>
      ...
      {selectedEdit && (
        <div className="cnic-modal-overlay" onClick={() => setSelectedEdit(null)}>
          <div className="cnic-modal" style={{ maxWidth: "560px", width: "90%" }} onClick={(e) => e.stopPropagation()}>
